...
users.showData = function () {
    $.ajax({
        url: "https://localhost:44368/BandUser/Get",
        method: "GET",
                ...
                $('#tbUser tbody').append(
                    `
                        <tr>
                            ...
                            <td>${item.userName}</td>
                            <td>${item.email}</td>
                            <td ><a class="btn btn-danger btn-sm">${item.isDeleted ? "Bị Cấm" : "Hoạt Động"}</a></td>
                            <td class='text-right'>
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                ...
                $.ajax({
                    url: "https://localhost:44368/BandUser/ChangeIsDeleted",
                    method: "PUT",
                    dataType: "json",
